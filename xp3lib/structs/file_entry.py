...
class XP3FileEncryption:
    ...
    def read_from(cls, buffer: BufferedReader, name: bytes = b'eliF'):
        ...
        # Some games (e.g. ATRI) omit the 2-byte padding; reading by size supports both.
        path_bytes = size - 6  # 4(adler) + 2(path_length)
        raw = buffer.read(path_bytes)
        file_path = raw[:file_path_length * 2].decode('utf-16le', errors='replace')
...
class XP3FileInfo:
    ...
    def read_from(cls, buffer: BufferedReader):
        ...
        # Some games (e.g. ATRI) omit the 2-byte padding; reading by size supports both.
        path_bytes = size - 22  # 4(flags) + 8(uncompressed) + 8(compressed) + 2(path_length)
        raw = buffer.read(path_bytes)
        file_path = raw[:file_path_length * 2].decode('utf-16le', errors='replace')
